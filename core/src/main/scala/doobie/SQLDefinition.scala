...
trait SQLDefinition[A] extends TypedMultiFragment[A] { self =>
  ...
  /** To allow widening the type of [[Column]] and similar ones to
    ...
    * `(Column[Int], Column[String])` is not the same thing as `(SQLResult[Int],
    * SQLResult[String])`.
    */
  ...
  /** Prefixes all column names with `EXCLUDED`, which is a special SQL table
    ...
    *
    * Example: {{{ sql""" ${t.Row.insertSqlFor(t)} ON CONFLICT (${t.userId},
...
object SQLDefinition {
  ...
  given toSingleFragment[A]
      : Conversion[SQLDefinition[A], SingleFragment[Nothing]] = d =>
    SingleFragment(d.sql)
